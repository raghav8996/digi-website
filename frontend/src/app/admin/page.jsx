...
export default function AdminDashboardPage() {
  ...
  if (!checked || !admin) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-[#4a4a55] text-sm">
        Loading dashboard…
      </div>
    );
  }
      ...
      <div className="border-b border-black/8 bg-white/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-5 md:px-10 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-[0.22em] text-[#83838f]">Admin</p>
              <p className="font-display font-black text-sm">DigiConnect CMS</p>
            </div>
          </Link>
          <div className="flex items-center gap-3">
            <span className="hidden sm:block text-xs text-[#83838f]">{admin?.email}</span>
            <Link
              href="/"
              data-testid="admin-view-site"
              className="text-xs font-semibold text-[#0f0f11]/70 hover:text-[#0f0f11] px-3 py-2 rounded-lg border border-black/8"
            >
              View site
            </Link>
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-lg bg-black/[0.05] border border-black/8 hover:bg-black/[0.08]"
            >
      ...
      {toast && (
        <div
          ...
          className={`fixed bottom-6 left-1/2 -translate-x-1/2 px-5 py-3 rounded-full text-sm font-bold z-50 ${
            toast.ok ? "bg-[#7a1b2e] text-white" : "bg-red-500 text-[#0f0f11]"
          }`}
        >
          {toast.msg}
        </div>
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-bold border transition-colors ${
        active
          ? "bg-[#7a1b2e] text-white border-[#7a1b2e]"
          : "bg-black/[0.03] text-[#0f0f11]/70 border-black/8 hover:text-[#0f0f11]"
      }`}
    >
      <Icon size={14} /> {label}
    </button>
