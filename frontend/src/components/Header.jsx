...
export default function Header() {
  ...
  return (
    <header
      ...
      className={`sticky top-0 z-40 transition-shadow duration-200 bg-[#050505] ${
    ...
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between px-5 md:px-10 py-4">
        ...
        <div className="hidden md:block">
          <Link
            ...
            href="/stores"
            className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-bold bg-[#ffffff] text-[#0a0a0b] hover:bg-white transition-colors"
          >
            Visit a Store
          </Link>
        </div>
      ...
      {open && (
        <div
          ...
          className="md:hidden border-t border-white/10 bg-[#050505] px-5 py-4 space-y-2"
        >
          {NAV.map((item) => (
            <Link
              key={item.to}
              ...
              className={`block px-4 py-3 rounded-xl text-base font-medium ${
                isActive(item.to) ? "bg-white/10 text-white" : "text-[#a1a1a6]"
              }`}
            >
              {item.label}
            </Link>
          ...
          <Link
            ...
            href="/stores"
            className="block text-center rounded-full px-5 py-3 text-sm font-bold bg-[#ff2d7a] text-white"
          >
            Visit a Store
          </Link>
        </div>
